...
The optional `ltm` object in `website-tiers.json` is generated by the updated bot. Its definitions contain each LTM name and icon, `active` identifies the current selection and its dates, and its player results remain after the mode expires. LTM points count in Overall permanently.

LTM uses the same `POINTS_BY_TIER` table and peak rule as every normal gamemode, so HT1 through LT5 always produce the same score in both places.

The LTM navigation tab uses `assets/ltm.svg`. It first shows every defined LTM; selecting one opens its five-column tier table. Each Overall row with saved LTM results has one LTM button using the same icon. That button opens a separate dialog containing only the LTMs tested by that player, with the individual LTM icon on the left and tier on the right. Players without an LTM result do not receive this button.

Upload `assets/ltm.svg` together with `index.html`, `script.js`, `tier-display.js`, and `newstyle.css`. The page remains compatible with an older JSON file that has no `ltm` field; it shows an empty LTM state until the bot publishes the first definition.
